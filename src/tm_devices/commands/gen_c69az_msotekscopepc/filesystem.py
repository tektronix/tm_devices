...
class FilesystemMountTekdrive(SCPICmdWrite, SCPICmdReadWithArguments):
    ...
    Info:
        - ``Power Cycle`` - Select to unmount the TekDrive after power cycling the oscilloscope.
          There is no time restriction when this option is selected.
        - ``Never`` - Select to mount the TekDrive connection permanently.
        - ``Custom`` - Select to disconnect the TekDrive after a chosen duration. The default
