...
class VerticalDeskewStatic(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        - Using the ``.query()`` method will send the ``VERTical:DESKew:STATIC?`` query.
        - Using the ``.verify(value)`` method will send the ``VERTical:DESKew:STATIC?`` query and
          raise an AssertionError if the returned value does not match ``value``.
...
class VerticalDeskew(SCPICmdRead):
    ...
    def static(self) -> VerticalDeskewStatic:
        ...
        Usage:
            - Using the ``.query()`` method will send the ``VERTical:DESKew:STATIC?`` query.
            - Using the ``.verify(value)`` method will send the ``VERTical:DESKew:STATIC?`` query
              and raise an AssertionError if the returned value does not match ``value``.
