...
class AliasDeleteName(SCPICmdWrite):
    ...
    Info:
        - ``<QString>`` is the name of the alias to remove. Using ``ALIas:DELEte:NAMewithout``
          specifying an alias causes an exception error.<QString> must be apreviously defined alias.
    """
...
class AliasDelete(SCPICmdWrite, SCPICmdRead):
    ...
    Info:
        - ``<QString>`` is the name of the alias to be removed. Using ``ALIas:DELEte`` without
          specifying an alias causes an execution error. <QString> must be apreviously defined
          value.
    ...
    def name(self) -> AliasDeleteName:
        ...
        Info:
            - ``<QString>`` is the name of the alias to remove. Using ``ALIas:DELEte:NAMewithout``
              specifying an alias causes an exception error.<QString> must be apreviously defined
              alias.
        """
        return self._name
...
class AliasDefine(SCPICmdWrite, SCPICmdRead):
    ...
    Info:
        - ``<QString>`` assigns a sequence of program messages to an alias label. The<QString> is
          argument is formatted as: <QString><,>{<QString>\|<Block>}. Thefirst <QString> is the
          alias label. This label cannot be a command name. Labelsmust start with a letter and can
          contain only letters, numbers, and underscores;other characters are not allowed. The label
          must be less than or equal to 12 characters. The second <QString> or <Block> is a complete
          sequence of programmessages. The messages can contain only valid commands that must be
          separatedby semicolons and must follow all rules for concatenating commands. Thesequence
...
class Alias(SCPICmdWrite, SCPICmdRead):
    ...
    def define(self) -> AliasDefine:
        r"""Return the ``ALIas:DEFine`` command.
        ...
        Info:
            - ``<QString>`` assigns a sequence of program messages to an alias label. The<QString>
              is argument is formatted as: <QString><,>{<QString>\|<Block>}. Thefirst <QString> is
              the alias label. This label cannot be a command name. Labelsmust start with a letter
              and can contain only letters, numbers, and underscores;other characters are not
              allowed. The label must be less than or equal to 12 characters. The second <QString>
              or <Block> is a complete sequence of programmessages. The messages can contain only
              valid commands that must be separatedby semicolons and must follow all rules for
              concatenating commands. Thesequence must be less than or equal to 256 characters.
        """
        return self._define
