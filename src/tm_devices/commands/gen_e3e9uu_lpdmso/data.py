...
class DataStop(SCPICmdWrite, SCPICmdRead):
    ...
    Description:
        - This command sets or queries the last data point that will be transferred when using the
          CURVe?? query. When using the CURVe? command, ``DATa:STOP`` is ignored. This command
          allows for the transfer of partial waveforms to the controller. If <NR1> is greater than
          the record length, then data will be transferred up to the record length. If both
...
class DataResample(SCPICmdWrite, SCPICmdRead):
    ...
    Info:
        - ``<NR1>`` is the resampling rate. The default value is 1, whichmeans every sampleis
          returned. A value of 2 returns every other sample, while a value of 3 returnsevery third
          sample, and so on.
    """
...
class Data(SCPICmdWrite, SCPICmdRead):
    ...
    def resample(self) -> DataResample:
        ...
        Info:
            - ``<NR1>`` is the resampling rate. The default value is 1, whichmeans every sampleis
              returned. A value of 2 returns every other sample, while a value of 3 returnsevery
    ...
    def stop(self) -> DataStop:
        ...
        Description:
            - This command sets or queries the last data point that will be transferred when using
              the CURVe?? query. When using the CURVe? command, ``DATa:STOP`` is ignored. This
              command allows for the transfer of partial waveforms to the controller. If <NR1> is
              greater than the record length, then data will be transferred up to the record length.
              If both ``DATa:STARt`` and ``DATa:STOP`` are greater than the record length, the last
              data point in the record is returned. ``DATa:STARt`` and ``DATa:STOP`` are order
