...
class Rst(SCPICmdWriteNoArguments):
    ...
    Description:
        - This command (no query form) resets the instrument to the factory default settings. This
          ...
          command). The state of keyword and enumeration verbosity (``:VERBose`` command). The
          Power-on Status Clear Flag (``*PSC`` command). The Event Status Enable Register (``*ESE``
          ...
          ``:CURVe?`` or ``:WAVFrm?`` queries (``:DATa:SOUrce`` command). The source waveform for
          the ``:CURVe?`` or ``:WAVFrm?`` queries (``:DATa:SOUrce`` command). The waveform data
          encoding for the ``:CURVe`` command or query or the ``:WAVFrm?`` query (``:DATa:ENCdg``
          command). The starting point for ``:CURVe?`` queries (``:DATa:STARt`` command). The ending
          point for ``:CURVe?`` queries (``:DATa:STOP`` command). All settings associated the
          ``:WFMInpre`` commands. All user settable settings associated with the WFMOutpre commands.
...
class Opc(SCPICmdWriteNoArguments, SCPICmdRead):
    ...
    Description:
        - This command generates the operation complete message in the Standard Event Status
          ...
          Synchronization Methods. Refer to the Oscilloscope operations that can generate OPC
