...
class Rst(SCPICmdWriteNoArguments):
    ...
    Description:
        - This command (no query form) resets the instrument to the factory default settings. This
          ...
          the ``*PUD`` command). The ``*RST The`` current working directory (:
          ...
          ``SOCKETServer:PROTOCol USBDevice``:CONFigure The destination reference waveform or file
          path for the : CURVe ``DATa:DESTination The`` source waveform for the : CURVe? WAVFrm?
          ``DATa:SOUrce The`` source waveform for the : CURVe? WAVFrm? ``DATa:SOUrce The`` waveform
          data encoding for the : CURVe WAVFrm? ``DATa:ENCdg The`` starting point for : CURVe?
          ``DATa:STARt CURVe``? ``DATa:STOP WFMInpre`` All user settable settings associated with
          the WFMOutpre ``*RST``
