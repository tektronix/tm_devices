...
Commands and Queries:
    ...
    - EYEMASK:MASK<x>:TESt:STATUS YEMASK:MASK<x>:TESt:STATUS?
    ```
"""
...
class EyemaskMaskItemTestStatus(SCPICmdWrite):
    ...
    Usage:
        - Using the ``.write(value)`` method will send the ``EYEMASK:MASK<x>:TESt:STATUS value``
          command.
...
class EyemaskMaskItemMaskfile(SCPICmdWrite, SCPICmdRead):
    ...
    Info:
        ...
        - ``<QString>`` is a quoted string that defines the file path that specifies the locationof
          the mask file to use, in the format '[<path>]<filename.ext>'. Specifying a pathis
          optional. If no path is entered, the instrument will search in the current
          workingdirectory as set in ``FILESystem:CWD``.
    """
...
class EyemaskMaskItemEnabled(SCPICmdWrite, SCPICmdRead):
    ...
    Info:
        - ``MASK<x>`` is the number of the specified mask plot.
        - ``ON`` enables the eye mask test.
        - ``OFF`` disables the eye mask test.
    """
    ...
    def maskfile(self) -> EyemaskMaskItemMaskfile:
        ...
        Info:
            ...
            - ``<QString>`` is a quoted string that defines the file path that specifies the
              locationof the mask file to use, in the format '[<path>]<filename.ext>'. Specifying a
              pathis optional. If no path is entered, the instrument will search in the current
              workingdirectory as set in ``FILESystem:CWD``.
        """
