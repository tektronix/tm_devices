...
class AutosetVerticalOptimize(SCPICmdWrite, SCPICmdRead):
    ...
    Info:
        - ``RESOlution`` uses as much of the ADC's (Analog to Digital Converter) range aspossible to
          provide the best vertical resolution and measurement accuracy, butwaveforms will overlap
          each other.
        - ``VISIBility`` vertically scales and positions waveforms so they are visuallyseparated
          from each other at the expense of vertical resolution and measurementaccuracy.
    """
...
class AutosetVertical(SCPICmdRead):
    ...
    def optimize(self) -> AutosetVerticalOptimize:
        ...
        Info:
            - ``RESOlution`` uses as much of the ADC's (Analog to Digital Converter) range
              aspossible to provide the best vertical resolution and measurement accuracy,
              butwaveforms will overlap each other.
            - ``VISIBility`` vertically scales and positions waveforms so they are visuallyseparated
              from each other at the expense of vertical resolution and measurementaccuracy.
        """
...
class Autoset(SCPICmdWrite, SCPICmdRead):
    ...
    Info:
        - ``EXECute`` autosets the displayed waveform; this is equivalent to pressing the frontpanel
          Autoset button.
