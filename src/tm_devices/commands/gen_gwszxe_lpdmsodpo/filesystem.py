...
class FilesystemRename(SCPICmdWrite):
    ...
    Info:
        - ``<old_file_path>,<new_file_path>`` is a quoted string that defines the file or folder
          name and path. If the path is within the current working directory, you need only specify
...
class FilesystemMountTekdrive(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        - Using the ``.query()`` method will send the ``FILESystem:MOUNT:TEKDrive?`` query.
        - Using the ``.verify(value)`` method will send the ``FILESystem:MOUNT:TEKDrive?`` query and
          raise an AssertionError if the returned value does not match ``value``.
...
class FilesystemMountDrive(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        - Using the ``.query()`` method will send the ``FILESystem:MOUNT:DRIVE?`` query.
        - Using the ``.verify(value)`` method will send the ``FILESystem:MOUNT:DRIVE?`` query and
          raise an AssertionError if the returned value does not match ``value``.
...
class FilesystemMount(SCPICmdRead):
    ...
    def drive(self) -> FilesystemMountDrive:
        ...
        Usage:
            - Using the ``.query()`` method will send the ``FILESystem:MOUNT:DRIVE?`` query.
            - Using the ``.verify(value)`` method will send the ``FILESystem:MOUNT:DRIVE?`` query
              and raise an AssertionError if the returned value does not match ``value``.
    ...
    def tekdrive(self) -> FilesystemMountTekdrive:
        ...
        Description:
            - This command mounts the TekDrive specified by the quoted string arguments. The
              ...
              restriction when this option is selected. Select Never to mount the TekDrive
              connection permanently. Select Custom to disconnect the TekDrive after a chosen
              duration. The default selection is Power Cycle. RestrictToCurrentIP restricts the
              connection to current network IP only. This may be used for additional network
              security. AutoDisconnectTime is the required time for the Auto Disconnect. The
              TekDrive gets disconnected automatically from the instrument after the specified time.
              The duration is in hours. The minimum is 0.25 hours and the maximum is 744 hours. The
              query form of this command returns whether or not the specified TekDrive is mounted.

        Usage:
            - Using the ``.query()`` method will send the ``FILESystem:MOUNT:TEKDrive?`` query.
            - Using the ``.verify(value)`` method will send the ``FILESystem:MOUNT:TEKDrive?`` query
              and raise an AssertionError if the returned value does not match ``value``.
            - Using the ``.write(value)`` method will send the ``FILESystem:MOUNT:TEKDrive value``
              command.
...
class FilesystemLdir(SCPICmdRead):
    ...
    Description:
        - Returns a comma separated list of every file, file size, type, modification date and time,
          and directory in the folder referred to by the ``FILESYSTEM:CWDFILESystem:CWD`` command.
          This is different than the ``:DIR`` query in that it provides a long output format with
          the file size, type, and modification date/time. Each entry is a semicolon separated list:
...
class FilesystemCwd(SCPICmdWrite, SCPICmdRead):
    ...
    Info:
        - ``<QString>`` is a quoted string that defines the current working; a directory name can be
...
class Filesystem(SCPICmdRead):
    ...
    def cwd(self) -> FilesystemCwd:
        ...
        Info:
            - ``<QString>`` is a quoted string that defines the current working; a directory name
    ...
    def ldir(self) -> FilesystemLdir:
        ...
        Description:
            - Returns a comma separated list of every file, file size, type, modification date and
              time, and directory in the folder referred to by the ``FILESYSTEM:CWDFILESystem:CWD``
              command. This is different than the ``:DIR`` query in that it provides a long output
              format with the file size, type, and modification date/time. Each entry is a semicolon
              separated list: < file name>;<type>;<size in bytes>;<date>;<time>
    ...
    def rename(self) -> FilesystemRename:
        ...
        Info:
            - ``<old_file_path>,<new_file_path>`` is a quoted string that defines the file or folder
              name and path. If the path is within the current working directory, you need only
