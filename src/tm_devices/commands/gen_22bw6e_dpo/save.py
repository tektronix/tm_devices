...
Commands and Queries:
    ...
    - SAVe:WAVEform:GATing {NONe|CURSors|SCREEN|RESAMPLE|}
...
class SaveWaveform(SCPICmdWrite, SCPICmdRead):
    ...
    Info:
        ...
        - ``_FREQ_VS_TIME`` saves the Frequency vs.
        - ``_PHASE_VS_TIME`` saves the Phase vs.
        - ``SV_BASEBAND_IQ`` saves the baseband I & Q data of the specified channel.
...
class SaveReport(SCPICmdRead):
    """The ``SAVe:REPOrt`` command tree.

    Usage:
        - Using the ``.query()`` method will send the ``SAVe:REPOrt?`` query.
        - Using the ``.verify(value)`` method will send the ``SAVe:REPOrt?`` query and raise an
...
class Save(SCPICmdRead):
    ...
    def mask(self) -> SaveMask:
        ...
        Description:
            - Saves the given Waveview Mask to the specified file. Use the format
              ...
              masks must be saved with a.xml extension, while tolerance masks must be saved with
              a.tol extension.
    ...
    def report(self) -> SaveReport:
        """Return the ``SAVe:REPOrt`` command tree.
