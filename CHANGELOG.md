...
- Added a `log_response_max_characters` configuration option and a matching `log_response_max_characters` argument on the `.query()`, `.query_binary()`, and `.query_raw_binary()` methods to truncate long command responses in the logs. Truncation is applied by the new `ResponseTruncationFilter`, which is attached to each handler created by `configure_logging()`.
